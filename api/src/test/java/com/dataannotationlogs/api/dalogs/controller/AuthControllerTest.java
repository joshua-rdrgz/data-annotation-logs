...
public class AuthControllerTest extends EmailTestBase {
  ...
  public void setup() {
    ...
    validLoginRequest =
        LoginAuthRequest.builder().email("john@example.com").password("password123").build();
  }
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post(
                "/api/v1/auth/verify?token={token}&userId={userId}", token, user.getId()))
        .andExpect(status().isOk());
  }
  ...
  public void verifyAccount_withInvalidToken_shouldReturnError() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post(
                "/api/v1/auth/verify?token={token}&userId={userId}", "invalid_token", user.getId()))
        .andExpect(status().isBadRequest());
  }
  ...
  public void resendVerificationEmail_withValidEmail_shouldSendEmail() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post(
                "/api/v1/auth/resend-verification?email={email}", "john@example.com"))
        .andExpect(status().isOk());
